defmodule KinshineWeb.UserAuthTest do
  use KinshineWeb.ConnCase, async: true

  alias Phoenix.LiveView
  alias Kinshine.Accounts
  alias Kinshine.Accounts.Scope
  alias KinshineWeb.UserAuth
  ...
  setup %{conn: conn} do
    conn =
      conn
      |> Map.replace!(:secret_key_base, KinshineWeb.Endpoint.config(:secret_key_base))
      |> init_test_session(%{})
  ...
  describe "logout_user/1" do
    ...
    test "broadcasts to the given live_socket_id", %{conn: conn} do
      ...
      KinshineWeb.Endpoint.subscribe(live_socket_id)
  ...
  describe "fetch_current_scope_for_user/2" do
    test "authenticates user from session", %{conn: conn, user: user} do
      ...
      assert conn.assigns.current_scope.user.userid == user.userid
  ...
  describe "on_mount :mount_current_scope" do
    ...
    test "assigns current_scope based on a valid user_token", %{conn: conn, user: user} do
      ...
      assert updated_socket.assigns.current_scope.user.userid == user.userid
    end
  ...
  describe "disconnect_sessions/1" do
    test "broadcasts disconnect messages for each token" do
      ...
      for %{token: token} <- tokens do
        KinshineWeb.Endpoint.subscribe("users_sessions:#{Base.url_encode64(token)}")
      end
